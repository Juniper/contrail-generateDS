...
class IFMapApiGenerator(object):
    ...
    def Generate(self, gen_filepath_pfx):
        ...
        # These produce classes per type
        ...
        # These produce class common to all types
    ...
    def _generate_common_classes(self, gen_filepath_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            # init args are name, props, parent_obj
            ...
            write(gen_file, "    def __init__(%s, **kwargs):" %(init_args))
            ...
            if parents:
                ...
                write(gen_file, "            if not parent_obj.get_%ss():" %(method_name))
                write(gen_file, "                parent_obj.%ss = []" %(method_name))
                write(gen_file, "            parent_obj.%ss.append(self)" %(method_name))
            ...
            write(gen_file, "    def factory(cls, **kwargs):")
            write(gen_file, "        parent_type = None")
            write(gen_file, "        if 'parent_type' in kwargs:")
            write(gen_file, "            parent_type = kwargs['parent_type']")
            write(gen_file, "        obj = %s(parent_type = parent_type)" %(class_name))
            write(gen_file, "        obj.__dict__.update(kwargs)")
            write(gen_file, "        return obj")
            write(gen_file, "    #end factory")
            write(gen_file, "")
            ...
            for prop in ident.getProperties():
                ...
                write(gen_file, "    def set_%s(self, %s):" %(prop_name, prop_name))
                ...
                write(gen_file, '        ')
                write(gen_file, '        """')
                write(gen_file, "        return getattr(self, '%s', None)" %(prop_name))
            ...
            for link_info in ident.getLinksInfo():
                ...
                if link.getXsdType(): # link with attr
                   ...
                   set_list_val = "[{'to':ref_obj_list[i], \
                                     'attr':ref_data_list[i]} for i in range(len(ref_obj_list))]"
    ...
    def _generate_client_classes(self, gen_filepath_pfx, gen_filename_pfx):
        ...
        write(gen_file, "# AUTO-GENERATED file from %s. Do Not Edit!" \
              %(self.__class__.__name__))
        write(gen_file, "")
        write(gen_file, "from vnc_api.gen.%s_common import *" %(gen_filename_pfx))
        write(gen_file, "")

        for ident in self._non_exclude_idents():
            class_name = CamelCase(ident.getName())
            write(gen_file, "class %sClientGen(%s):" %(class_name, class_name))
            write(gen_file, "    create_uri = ''")
            write(gen_file, "    resource_uri_base = {}")
            write(gen_file, "    def __init__(self):")
            write(gen_file, "        pass")
            write(gen_file, "    #end __init__")
            write(gen_file, "#end class %sClientGen" %(class_name))
            write(gen_file, "")
    ...
    def _generate_server_classes(self, gen_filepath_pfx, gen_filename_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "")
            write(gen_file, "    @classmethod")
            write(gen_file, "    def http_put(cls, id, obj):")
            write(gen_file, "        return True, ''")
            ...
            write(gen_file, "")
            write(gen_file, "    @classmethod")
            write(gen_file, "    def http_delete(cls, id, obj):")
            write(gen_file, "        return True, ''")
            write(gen_file, "    #end http_delete")
            write(gen_file, "")
            write(gen_file, "    @classmethod")
            write(gen_file, "    def http_post_collection(cls, tenant_name, obj):")
            write(gen_file, "        return True, ''")
            write(gen_file, "    #end http_post")
            write(gen_file, "")
            # IFMAP methods
            write(gen_file, "    @classmethod")
            write(gen_file, "    def _db_read(cls, id):")
            write(gen_file, "        pass")
            write(gen_file, "    #end _db_read")
            write(gen_file, "")
            write(gen_file, "    @classmethod")
            write(gen_file, "    def _db_create(cls, tenant_name, obj):")
            write(gen_file, "        pass")
            write(gen_file, "    #end _db_create")
            write(gen_file, "")
    ...
    def _generate_test_classes(self, gen_filepath_pfx, gen_filename_pfx):
        ...
        write(gen_file, "from vnc_api.gen.%s_common import *" %(gen_filename_pfx))
        write(gen_file, "")
        write(gen_file, "from vnc_api.gen.resource_xsd import *")
...
'''
    ...
    def _generate_client_impl(self, gen_fname, gen_type_pfx):
        ...
        write(gen_file, "from vnc_api.gen.%s_xsd import *" %(gen_type_pfx))
        write(gen_file, "from vnc_api.gen.%s_client import *" %(gen_type_pfx))
        ...
        for ident in self._non_exclude_idents():
            ...
            method_name = ident_name.replace('-', '_')
            camel_name = CamelCase(ident_name)
            write(gen_file, "        self._type_to_class['%s'] = %s" %(method_name, camel_name))
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "        json_param = json.dumps(obj,")
            ...
            write(gen_file, "        uri = %sClientGen.resource_uri_base['%s'] + '/' + id" \
                                           %(camel_name, ident_name))
            write(gen_file, "")
            write(gen_file, "        content = self._request_server(rest.OP_GET, uri)")
            write(gen_file, "")
            write(gen_file, "        obj_dict = json.loads(content)['%s']" %(ident_name))
            write(gen_file, "        obj_dict['name'] = obj_dict['fq_name'][-1]")
            for prop in ident.getProperties():
                prop_name = prop.getName().replace('-', '_')
                complex_type = prop.getCType()
                xsd_type = prop.getXsdType()
                if complex_type and xsd_type:
                    # convert wire read dict to obj
                    write(gen_file, "        if obj_dict.has_key('%s'):" %(prop_name))
                    write(gen_file, "            prop_obj = %s(**obj_dict['%s'])" \
                                                            %(xsd_type, prop_name))
                    write(gen_file, "            obj_dict['%s'] = prop_obj" %(prop_name))
                    if prop.getName() not in prop_name_to_xsd_type_dict:
                        prop_name_to_xsd_type_dict[prop.getName()] = xsd_type
                write(gen_file, "")

            write(gen_file, "        # fill ref links")
            for link_info in ident.getLinksInfo():
                ...
                to_name = to_ident.getName().replace('-', '_')
                link = ident.getLink(link_info)
                complex_type = link.getCType()
                xsd_type = link.getXsdType()
                if complex_type and xsd_type:
                    # convert wire read dict to obj
                    write(gen_file, "        if obj_dict.has_key('%s_refs'):" %(to_name))
                    write(gen_file, "            list_len = len(obj_dict['%s_refs'])" %(to_name))
                    write(gen_file, "            for idx in range(list_len):")
                    write(gen_file, "                ref = obj_dict['%s_refs'][idx]" %(to_name))
                    write(gen_file, "                if ref['attr']:")
                    write(gen_file, "                    data_obj = %s(**ref['attr'])" %(xsd_type))
                    write(gen_file, "                else:")
                    write(gen_file, "                    data_obj = None")
                    write(gen_file, "                ref_dict = {}")
                    write(gen_file, "                ref_dict['to'] = ref['to']")
                    write(gen_file, "                ref_dict['attr'] = data_obj")
                    write(gen_file, "                ref_dict['uuid'] = ref['uuid']")
                    write(gen_file, "                ref_dict['href'] = ref['href']")
                    write(gen_file, "                obj_dict['%s_refs'][idx] = ref_dict" %(to_name))
                    write(gen_file, "")

            write(gen_file, "        # fill back ref links")
            for back_link_info in ident.getBackLinksInfo():
                if not ident.isLinkRef(back_link_info):
                    continue
                from_ident = ident.getBackLinkFrom(back_link_info)
                ...
                complex_type = back_link.getCType()
                xsd_type = back_link.getXsdType()
                if complex_type and xsd_type:
                    # convert wire read dict to obj
                    write(gen_file, "        if obj_dict.has_key('%s_back_refs'):" %(from_name))
                    write(gen_file, "            list_len = len(obj_dict['%s_back_refs'])" %(from_name))
                    write(gen_file, "            for idx in range(list_len):")
                    write(gen_file, "                back_ref = obj_dict['%s_back_refs'][idx]" %(from_name))
                    write(gen_file, "                if back_ref['attr']:")
                    write(gen_file, "                    data_obj = %s(**back_ref['attr'])" %(xsd_type))
                    write(gen_file, "                else:")
                    write(gen_file, "                    data_obj = None")
                    write(gen_file, "                back_ref_dict = {}")
                    write(gen_file, "                back_ref_dict['to'] = back_ref['to']")
                    write(gen_file, "                back_ref_dict['attr'] = data_obj")
                    write(gen_file, "                back_ref_dict['uuid'] = back_ref['uuid']")
                    write(gen_file, "                back_ref_dict['href'] = back_ref['href']")
                    write(gen_file, "                obj_dict['%s_back_refs'][idx] = back_ref_dict" %(from_name))
                    write(gen_file, "")

            write(gen_file, "        return %s.factory(**obj_dict)" %(camel_name))
            write(gen_file, "    #end %s_read" %(method_name))
            write(gen_file, "")
            ...
            write(gen_file, '        """')
            # TODO below 2 lines needed? if so what for oper/cfg objects
            #write(gen_file, "        # Fixup to always have specified tenant")
            #write(gen_file, "        obj._fq_name[0] = self._tenant_name")
            ...
            if parents:
                ...
                write(gen_file, "            filt['parent_fq_name_str'] = parent_fq_name_str")
                write(gen_file, "        elif parent_id:")
                write(gen_file, "            filt['parent_id'] = parent_id")
            ...
            write(gen_file, "    def get_default_%s_id(self):" %(method_name))
            write(gen_file, '        """Return UUID of default %s."""' %(ident_name))
            write(gen_file, "        return self.fq_name_to_id('%s', %s().get_fq_name())" %(ident_name, camel_name))
            write(gen_file, "    #end get_default_%s_delete" %(method_name))
            write(gen_file, "")
    ...
    def _generate_server_impl(self, gen_fname, gen_type_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "        else:")
            write(gen_file, "            obj_fields = None")
            ...
            write(gen_file, "        r_class = self._resource_classes.get('%s', None)" \
                                                                           %(ident_name))
            write(gen_file, "        if r_class:")
            write(gen_file, "            (ok, msg) = r_class.http_put(id, obj_dict)")
            write(gen_file, "            if not ok:")
            write(gen_file, "                self.config_object_error(id, None, '%s', 'http_put', msg)" % (method_name))
            write(gen_file, "                abort(409, msg)")
            write(gen_file, "") 
            write(gen_file, "        db_conn = self._db_conn")
            write(gen_file, "")
            ...
            write(gen_file, "             db_conn.dbe_update('%s', obj_ids, obj_dict)" %(ident_name))
            write(gen_file, "        if not ok:")
            ...
            write(gen_file, "        obj_ids = {'uuid': id}")
            write(gen_file, "        (read_ok, read_result) = db_conn.dbe_read('%s', obj_ids)" %(ident_name))
            ...
            write(gen_file, "                parent_obj = %s.factory(**read_result)" %(camel_name))
            write(gen_file, "                self._%s_delete_default_children(parent_obj)" %(method_name))
            write(gen_file, "")
            write(gen_file, "                r_class.http_delete(id, read_result)")
            write(gen_file, "            #end if read_ok") 
            write(gen_file, "")
            write(gen_file, "        (ok, del_result) = \\")
            write(gen_file, "             db_conn.dbe_delete('%s', obj_ids)" %(ident_name))
            write(gen_file, "        if not ok:")
            ...
            write(gen_file, "            abort(code, result)")
            write(gen_file, "") 
            ...
            write(gen_file, "        r_class = self._resource_classes.get('%s', None)" \
                                                                           %(ident_name))
            write(gen_file, "        if r_class:")
            write(gen_file, "            (ok, result) = r_class.http_post_collection(tenant_name, obj_dict)")
            write(gen_file, "            if not ok:")
            write(gen_file, "                (code, msg) = result")
            ...
            write(gen_file, "        env = request.headers.environ")
            write(gen_file, "        tenant_name = env.get(hdr_server_tenant(), 'default-project')")
            write(gen_file, "        parent_uuid = None")
            write(gen_file, "        resource_id = '%ss'" %(method_name))
            ...
            write(gen_file, "            parent_uuid = str(uuid.UUID(request.query.parent_id))")
            write(gen_file, "")
            write(gen_file, "        if parent_uuid:")
            write(gen_file, "            resource_id = parent_uuid")      
            write(gen_file, "")
            write(gen_file, "        # common handling for all resource get")
            write(gen_file, "        (ok, result) = self._get_common(request, parent_uuid)")
            write(gen_file, "        if not ok:")
            write(gen_file, "            (code, msg) = result")
            ...
            write(gen_file, "        db_conn = self._db_conn")
            write(gen_file, "        (ok, result) = \\")
            write(gen_file, "             db_conn.dbe_list('%s', parent_uuid)" %(ident_name))
            write(gen_file, "")
            write(gen_file, "        if not ok:")
            write(gen_file, "            self.config_object_error(None, None, '%ss', 'http_get_collection', result)" % (method_name))
            write(gen_file, "            abort(404, result)")
            write(gen_file, "") 
            write(gen_file, "        fq_names = result") 
            write(gen_file, "        obj_dicts = []")
            write(gen_file, "        for fq_name in fq_names:")
            write(gen_file, "            obj_dict = {}")
            write(gen_file, "            id = db_conn.fq_name_to_uuid('%s', fq_name)" %(method_name))
            write(gen_file, "            obj_dict['uuid'] = id")
            write(gen_file, "            obj_dict['fq_name'] = fq_name")
            write(gen_file, "            obj_dict['href'] = self.generate_url('%s', id)" %(ident_name))
            ...
            write(gen_file, "")
            write(gen_file, "    def _%s_delete_default_children(self, parent_obj):" %(method_name))
            for (child_ident, child_link_info) in child_idents:
                child_method = child_ident.getName().replace('-', '_')
                ...
                write(gen_file, "            has_infos = parent_obj.get_%ss()" %(child_method))
        ...
        for ident in self._non_exclude_idents():
            ...
            for (meta_ns_name, xsd_info, id_name) in meta_info:
                ...
                if meta_name in ref_links: # ref link obj field-name = _<to_name>_refs
                    ...
                    write(gen_file, "                    return (False, err_msg)")
                    write(gen_file, "")
                    write(gen_file, "                ref_url = self._db_client_mgr.generate_url('%s', ref_uuid)" %(to_name))
                    ...
                    write(gen_file, "                    return (False, err_msg)")
                    write(gen_file, "")
                    write(gen_file, "                has_url = self._db_client_mgr.generate_url('%s', has_uuid)" %(to_name))
                    ...
                    write(gen_file, "                    return (False, err_msg)")
                    write(gen_file, "")
                    write(gen_file, "                back_ref_url = self._db_client_mgr.generate_url('%s', back_ref_uuid)" %(from_name))
            ...
            # IFMAP_UPDATE
            write(gen_file, "    def _ifmap_%s_update(self, ifmap_id, old_obj_dict, new_obj_dict):" \
                                            %(method_name))
            ...
            for link_info in ident.getLinksInfo():
                ...
                to_ident = ident.getLinkTo(link_info)
                to_name = to_ident.getName().replace('-', '_')
                write(gen_file, "        if ('%s_refs' in old_obj_dict):" %(to_name))
                write(gen_file, "            old_refs = [tuple(ref['to']) for ref in old_obj_dict['%s_refs']]" %(to_name))
                write(gen_file, "        else:")
                write(gen_file, "            old_refs = []")
                write(gen_file, "        if ('%s_refs' in new_obj_dict):" %(to_name))
                write(gen_file, "            if new_obj_dict['%s_refs'] == None:" %(to_name))
                write(gen_file, "                new_refs = []")
                write(gen_file, "            else:")
                write(gen_file, "                new_refs = [tuple(ref['to']) for ref in new_obj_dict['%s_refs']]" %(to_name))
                write(gen_file, "        else: # no ref key in new_obj")
                write(gen_file, "            new_refs = old_refs")
                write(gen_file, "")
                write(gen_file, "        old_set = set(old_refs)")
        ...
        for ident in self._non_exclude_idents():
            ...
            for prop in ident.getProperties():
                ...
                if prop_method_name == 'id_perms':
                    write(gen_file, "        field['created'] = datetime.datetime.now().isoformat()")
            ...
            # CASSANDRA_READ
            write(gen_file, "    def _cassandra_%s_read(self, obj_uuid, field_names = None):" %(method_name))
            write(gen_file, "        obj_uuid_cf = self._obj_uuid_cf")
            write(gen_file, "        obj_cols_iter = obj_uuid_cf.xget(obj_uuid, include_timestamp = True)")
            write(gen_file, "        # TODO optimize this if needed")
            write(gen_file, "        obj_cols = {}")
            write(gen_file, "        for col in obj_cols_iter:")
            write(gen_file, "            # col-name = value + tstamp")
            write(gen_file, "            obj_cols[col[0]] = {'value': col[1][0], 'tstamp': col[1][1]}")
            write(gen_file, "            # TODO use compile RE")
            write(gen_file, "            if re.match('parent:', col[0]):")
            write(gen_file, "                (_, _, parent_uuid) = col[0].split(':')")
            write(gen_file, "")
            write(gen_file, "        if not obj_cols:")
            write(gen_file, "            raise cfgm_common.exceptions.NoIdError(obj_uuid)")
            write(gen_file, "")
            write(gen_file, "        result = {}")
            write(gen_file, "        # Name info")
            write(gen_file, "        result['fq_name'] = json.loads(obj_cols['fq_name']['value'])")
            if parents:
                write(gen_file, "        if 'parent_type' in obj_cols:")
                write(gen_file, "            # non config-root child")
                write(gen_file, "            parent_type = json.loads(obj_cols['parent_type']['value'])")
                write(gen_file, "            result['parent_type'] = parent_type")
                write(gen_file, "            try:")
                write(gen_file, "                result['parent_uuid'] = parent_uuid")
                write(gen_file, "                result['parent_href'] = self._db_client_mgr.generate_url(parent_type, parent_uuid)")
                write(gen_file, "            except cfgm_common.exceptions.NoIdError:")
                if ident.isDerived():
                    write(gen_file, "                # ok for parent to go away since we are machine generated")
                    write(gen_file, "                pass")
                else:
            ...
            write(gen_file, "                result[prop_name] = json.loads(obj_cols[col_name]['value'])")
            write(gen_file, "")
            write(gen_file, "            # TODO use compiled RE")
            write(gen_file, "            if re.match('children:', col_name):")
            write(gen_file, "                (_, child_type, child_uuid) = col_name.split(':')")
            write(gen_file, "                child_tstamp = obj_cols[col_name]['tstamp']")
            write(gen_file, "                try:")
            ...
            write(gen_file, "                self._read_ref(result, obj_uuid, ref_type, ref_uuid, obj_cols[col_name]['value'])")
            write(gen_file, "")
            write(gen_file, "            if re.match('backref:', col_name):")
            ...
            write(gen_file, "                                        obj_cols[col_name]['value'])")
            write(gen_file, "                except cfgm_common.exceptions.NoIdError:")
            write(gen_file, "                    continue")
            write(gen_file, "")
            ...
            if children_idents:
                for child_ident in children_idents:
                    ...
                    write(gen_file, "            sorted_children = sorted(result['%ss'], key = itemgetter('tstamp'))" %(child_method_name))
            ...
            # CASSANDRA_UPDATE
            write(gen_file, "    def _cassandra_%s_update(self, obj_uuid, dummy_old_obj_dict, new_obj_dict):" %(method_name))
            ...
            write(gen_file, "                    new_id_perms['last_modified'] = datetime.datetime.now().isoformat()")
            write(gen_file, "                    new_props['id_perms'] = new_id_perms")
            write(gen_file, "                    self._update_prop(bch, obj_uuid, 'id_perms', new_props)")
            ...
            # CASSANDRA_LIST
            write(gen_file, "    def _cassandra_%s_list(self, parent_uuid):" %(method_name))
            write(gen_file, "        children_fq_names = []")
            write(gen_file, "        if not parent_uuid:")
            write(gen_file, "            # grab all resources of this type")
            write(gen_file, "            obj_fq_name_cf = self._obj_fq_name_cf")
            write(gen_file, "            col_name_iter = obj_fq_name_cf.xget('%s')" %(method_name))
            write(gen_file, "            for (col_name, col_val) in col_name_iter:")
            write(gen_file, "                fq_name = col_name.split(':')[:-1]")
            write(gen_file, "                children_fq_names.append(fq_name)")
            write(gen_file, "        else:")
            ...
            write(gen_file, "            col_name_iter = obj_uuid_cf.xget(parent_uuid, column_start = col_start, \\")
            write(gen_file, "                                column_finish = col_fin, include_timestamp = True)")
            write(gen_file, "")
            write(gen_file, "            unsorted_fq_names = []")
            write(gen_file, "            for col in col_name_iter:")
            write(gen_file, "                col_name = col[0]")
            write(gen_file, "                col_val_ts = col[1]")
            ...
            write(gen_file, "                unsorted_fq_names.append({'fq_name': child_fq_name, 'tstamp': tstamp})")
            write(gen_file, "")
            write(gen_file, "                # sort children; TODO do this based on schema")
            write(gen_file, "                sorted_children = sorted(unsorted_fq_names, key = itemgetter('tstamp'))")
            write(gen_file, "                # re-write result's children without timestamp")
            write(gen_file, "                children_fq_names = [child['fq_name'] for child in sorted_children]")
            write(gen_file, "")
            write(gen_file, "        return (True, children_fq_names)")
