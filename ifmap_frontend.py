...
'''
    ...
    def _generate_server_impl(self, gen_fname, gen_type_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "")
            write(gen_file, "        ff_names = request.query.filter_field_names")
            write(gen_file, "        ff_values = request.query.filter_field_values")
            write(gen_file, "        if ff_names:")
            write(gen_file, "            ff_names = ff_names.split(',')")
            write(gen_file, "        if ff_values:")
            write(gen_file, "            ff_values = ff_values.split(',')")
            write(gen_file, "        if len(ff_names) != len(ff_values):")
            write(gen_file, "            abort(400, 'Filter field name and value lengths different')")
            write(gen_file, "")
            write(gen_file, "        if len(ff_names) and len(ff_values):")
    ...
    def _generate_cassandra_db_impl(self, gen_fname, gen_type_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "")
            write(gen_file, "            def filter_rows_parent_anchor(sort):")
            ...
            write(gen_file, "                    # build new filtered list instead of deleting from all_")
            write(gen_file, "                    # since some rows may not even have the filter field")
            write(gen_file, "                    filt_child_infos = {}")
            write(gen_file, "                    child_rows = obj_uuid_cf.multiget(all_child_infos.keys(),")
            write(gen_file, "                                           columns=filter_cols,")
            write(gen_file, "                                           column_count=10000000)")
            write(gen_file, "                    for row in child_rows:")
            write(gen_file, "                        for fname, fval in filter_fields:")
            write(gen_file, "                            if child_rows[row]['prop:%s' %(fname)] == fval:")
            write(gen_file, "                                filt_child_infos[row] = all_child_infos[row]")
            write(gen_file, "                else: # no filter specified")
            write(gen_file, "                    filt_child_infos = all_child_infos")
            write(gen_file, "")
            write(gen_file, "                if not sort:")
            write(gen_file, "                    return [(self.uuid_to_fq_name(child_uuid), child_uuid)")
            write(gen_file, "                            for child_uuid in filt_child_infos.keys()]")
            write(gen_file, "                sorted_children = sorted(filt_child_infos.values(), key = itemgetter('tstamp'))")
            write(gen_file, "                return [(self.uuid_to_fq_name(s_child['uuid']), s_child['uuid'])")
            write(gen_file, "                         for s_child in sorted_children]")
            ...
            write(gen_file, "                    # build new filtered list instead of deleting from all_")
            write(gen_file, "                    # since some rows may not even have the filter field")
            write(gen_file, "                    filt_backref_infos = {}")
            write(gen_file, "                    backref_rows = obj_uuid_cf.multiget(all_backref_infos.keys(),")
            write(gen_file, "                                           columns=filter_cols,")
            write(gen_file, "                                           column_count=10000000)")
            write(gen_file, "                    for row in backref_rows:")
            write(gen_file, "                        for fname, fval in filter_fields:")
            write(gen_file, "                            if backref_rows[row]['prop:%s' %(fname)] == fval:")
            write(gen_file, "                                filt_backref_infos[row] = all_backref_infos[row]")
            write(gen_file, "                else: # no filter specified")
            ...
            write(gen_file, "                        # build new filtered list instead of deleting from all_")
            write(gen_file, "                        # since some rows may not even have the filter field")
            write(gen_file, "                        filt_obj_infos = {}")
            write(gen_file, "                        obj_rows = obj_uuid_cf.multiget(all_obj_infos.keys(),")
            write(gen_file, "                                           columns=filter_cols,")
            write(gen_file, "                                           column_count=10000000)")
            write(gen_file, "                        for row in obj_rows:")
            write(gen_file, "                            for fname, fval in filter_fields:")
            write(gen_file, "                                if obj_rows[row]['prop:%s' %(fname)] == fval:")
            write(gen_file, "                                    filt_obj_infos[row] = all_obj_infos[row]")
            write(gen_file, "                    else: # no filters specified")
            write(gen_file, "                        filt_obj_infos = all_obj_infos")
            write(gen_file, "")
            write(gen_file, "                    return [(self.uuid_to_fq_name(obj_uuid), obj_uuid) for obj_uuid in filt_obj_infos.keys()]")
