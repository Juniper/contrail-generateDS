...
'''
    ...
    def _generate_client_impl(self, gen_fname, gen_type_pfx):
        ...
        write(gen_file, "# AUTO-GENERATED file from %s. Do Not Edit!" \
              %(self.__class__.__name__))
        write(gen_file, "")
        write(gen_file, "from cfgm_common import jsonutils as json")
        ...
        for ident in self._non_exclude_idents():
            ...
            if ident.getReferences():
                list_args = list_args + ", back_ref_id = None"
            list_args = list_args + ", obj_uuids = None, fields = None, detail = False, count = False, filters = None"
            write(gen_file, "    def %ss_list(%s):" \
                                     %(method_name, list_args))
            if parents:
                write(gen_file, '        """List all %ss.' % (ident_name))
            ...
            resource_list_args += "obj_uuids=obj_uuids, fields=fields, detail=detail, count=count, filters=filters"
    ...
    def _generate_server_impl(self, gen_fname, gen_type_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "                abort(404, 'No %s object found for id %%s' %%(id))" %(ident_name))
            write(gen_file, "            obj_ids = {'uuid': id}")
            write(gen_file, "            (read_ok, read_result) = self._db_conn.dbe_read(key, obj_ids)")
            ...
            write(gen_file, "")
            write(gen_file, "        try:")
            write(gen_file, "            self._extension_mgrs['resourceApi'].map_method('post_%s_update', id, obj_dict, read_result)" %(method_name))
            write(gen_file, "        except Exception as e:")
            write(gen_file, "            pass")
            write(gen_file, "")
            write(gen_file, "        return {'%s': rsp_body} " %(ident_name))
            ...
            write(gen_file, "        except NoIdError as e:")
            write(gen_file, "            ok = False")
            write(gen_file, "            code = 404")
            write(gen_file, "            del_result = 'ID %s does not exist' % (id)")
            write(gen_file, "        except Exception as e:")
            write(gen_file, "            ok = False")
            write(gen_file, "            code = 409")
            write(gen_file, "            del_result = str(e)")
            write(gen_file, "        if not ok:")
            write(gen_file, "            for fail_cleanup_callable, cleanup_args in cleanup_on_failure:")
            write(gen_file, "                    fail_cleanup_callable(*cleanup_args)")
            write(gen_file, "            self.config_object_error(id, None, '%s', 'http_delete_gen_7', del_result)" % (method_name))
            write(gen_file, "            abort(code, del_result)")
            write(gen_file, "")
            write(gen_file, "        try:")
            ...
            write(gen_file, "            if 'fields' in request.query:")
            write(gen_file, "                req_fields = request.query.fields.split(',')")
            write(gen_file, "            else:")
            write(gen_file, "                req_fields = []")
            write(gen_file, "            if not self.is_admin_request():")
            write(gen_file, "                obj_ids_list = [{'uuid': obj_uuid} for _, obj_uuid in fq_names_uuids]")
            write(gen_file, "                obj_fields = [u'id_perms'] + req_fields")
            ...
            write(gen_file, "                        for field in req_fields:")
            write(gen_file, "                            try:")
            write(gen_file, "                                obj_dict[field] = obj_result[field]")
            write(gen_file, "                            except KeyError:")
            write(gen_file, "                                pass")
            write(gen_file, "                        obj_dicts.append(obj_dict)")
            write(gen_file, "            else:")
            write(gen_file, "                obj_results = {}")
            write(gen_file, "                if req_fields:")
            write(gen_file, "                    obj_ids_list = [{'uuid': obj_uuid} for _, obj_uuid in fq_names_uuids]")
            write(gen_file, "                    (ok, result) = db_conn.dbe_read_multi('%s', obj_ids_list, req_fields)" %(ident_name))
            write(gen_file, "                    if ok:")
            write(gen_file, "                        obj_results = dict((elem['uuid'], elem) for elem in result)")
            ...
            write(gen_file, "                    for field in req_fields:")
            write(gen_file, "                       try:")
            write(gen_file, "                           obj_dict[field] = obj_results[obj_uuid][field]")
            write(gen_file, "                       except KeyError:")
            write(gen_file, "                           pass")
            write(gen_file, "                    obj_dicts.append(obj_dict)")
    ...
    def _generate_ifmap_db_impl(self, gen_fname, gen_type_pfx):
        ...
        write(gen_file, "from cfgm_common.imid import escape")
        write(gen_file, "from %s_xsd import *" %(gen_type_pfx))
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "        if my_imid is None or parent_imid is None:")
            write(gen_file, "            return (False, (my_imid, parent_imid))")
            ...
            write(gen_file, "            metas_to_read = set(all_metas) & set(field_names.keys())")
            write(gen_file, "")
            ...
            write(gen_file, "        props = ['%s']" % "', '".join(props))
            write(gen_file, "        for prop in props:")
            write(gen_file, "            prop_m = prop.replace('-', '_')")
            write(gen_file, "            if prop in existing_metas and prop_m not in new_obj_dict:")
            write(gen_file, "                self._delete_id_self_meta(ifmap_id, 'contrail:'+prop)")

            links = []
            for link_info in ident.getLinksInfo():
                if not ident.isLinkRef(link_info):
                    continue
                links.append("'%s': '%s'" % (ident.getLink(link_info).getName(),
                                             ident.getLinkTo(link_info).getName()))
            
            write(gen_file, "        # remove refs that are no longer active")
            write(gen_file, "        delete_list = []")
            write(gen_file, "        refs = {%s}" % ',\n                '.join(links))
            write(gen_file, "        for meta, to_name in refs.items():")
            write(gen_file, "            old_set = set([m['id'] for m in existing_metas.get(meta, [])])")
            write(gen_file, "            new_set = set()")
            write(gen_file, "            to_name_m = to_name.replace('-', '_')")
            write(gen_file, "            for ref in new_obj_dict.get(to_name_m+'_refs', []):")
            write(gen_file, "                to_imid = self.fq_name_to_ifmap_id(to_name, ref['to'])")
            write(gen_file, "                new_set.add(to_imid)")
            write(gen_file, "")
            write(gen_file, "            for inact_ref in old_set - new_set:")
            write(gen_file, "                delete_list.append((inact_ref, 'contrail:'+meta))")
            write(gen_file, "")
            write(gen_file, "        if delete_list:")
            write(gen_file, "            self._delete_id_pair_meta_list(ifmap_id, delete_list)")
            write(gen_file, "")
            ...
            write(gen_file, "        parent_imid = obj_ids.get('parent_imid', None)")
            write(gen_file, "        existing_metas = self._ifmap_%s_read_to_meta_index(ifmap_id)" %(method_name))
            write(gen_file, "        meta_list = []")
            write(gen_file, "        for meta_name, meta_infos in existing_metas.items():")
            write(gen_file, "            for meta_info in meta_infos:")
            write(gen_file, "                ref_imid = meta_info.get('id')")
            write(gen_file, "                if ref_imid is None:")
            write(gen_file, "                    continue")
            write(gen_file, "                meta_list.append((ref_imid, 'contrail:'+meta_name))")
            write(gen_file, "")
            write(gen_file, "        if parent_imid:")
            write(gen_file, "            # Remove link from parent")
            write(gen_file, "            meta_list.append((parent_imid, None))")
            write(gen_file, "")
            write(gen_file, "        if meta_list:")
            write(gen_file, "            self._delete_id_pair_meta_list(ifmap_id, meta_list)")
            write(gen_file, "")
    ...
    def _generate_cassandra_db_impl(self, gen_fname, gen_type_pfx):
        ...
        write(gen_file, "")
        write(gen_file, "from pysandesh.gen_py.sandesh.ttypes import SandeshLevel")
        write(gen_file, "import cfgm_common.exceptions")
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "    def _cassandra_%s_count_children(self, obj_uuid, child_type):" %(method_name))
            write(gen_file, "        # if child_type = None, return")
            write(gen_file, "        if child_type is None:")
            write(gen_file, "            return (False, '')")
            write(gen_file, "")
            write(gen_file, "        obj_uuid_cf = self._obj_uuid_cf")
            write(gen_file, "        if child_type not in %s.children_fields:" %(ident_class_name))
            write(gen_file, "            return (False, '%s is not a valid children of " + ident_class_name + "' %(child_type))")
            write(gen_file, "")
            write(gen_file, "        col_start = 'children:'+child_type[:-1]+':'")
            write(gen_file, "        col_finish = 'children:'+child_type[:-1]+';'")
            write(gen_file, "        num_children = obj_uuid_cf.get_count(obj_uuid,")
            write(gen_file, "                                   column_start = col_start,")
            write(gen_file, "                                   column_finish = col_finish,")
            write(gen_file, "                                   max_count = 10000000)")
            write(gen_file, "        return (True, num_children)")
            write(gen_file, "    #end _cassandra_%s_count_children" %(method_name))
            write(gen_file, "")
            ...
            write(gen_file, "        if filters:")
            write(gen_file, "            fnames = filters.get('field_names', [])")
            write(gen_file, "            fvalues = filters.get('field_values', [])")
            write(gen_file, "            filter_fields = [(fnames[i], fvalues[i]) for i in range(len(fnames))]")
            write(gen_file, "        else:")
            write(gen_file, "            filter_fields = []")
            write(gen_file, "")
            write(gen_file, "        def filter_rows(coll_infos, filter_cols, filter_params):")
            write(gen_file, "            filt_infos = {}")
            write(gen_file, "            coll_rows = obj_uuid_cf.multiget(coll_infos.keys(),")
            write(gen_file, "                                   columns=filter_cols,")
            write(gen_file, "                                   column_count=10000000)")
            write(gen_file, "            for row in coll_rows:")
            write(gen_file, "                # give chance for zk heartbeat/ping")
            write(gen_file, "                gevent.sleep(0)")
            write(gen_file, "                full_match = True")
            write(gen_file, "                for fname, fval in filter_params:")
            write(gen_file, "                    if coll_rows[row]['prop:%s' %(fname)] != fval:")
            write(gen_file, "                        full_match = False")
            write(gen_file, "                        break")
            write(gen_file, "                if full_match:")
            write(gen_file, "                    filt_infos[row] = coll_infos[row]")
            write(gen_file, "            return filt_infos")
            write(gen_file, "        # end filter_rows")
            write(gen_file, "")
            write(gen_file, "        def get_fq_name_uuid_list(obj_uuids):")
            write(gen_file, "            ret_list = []")
            write(gen_file, "            for obj_uuid in obj_uuids:")
            write(gen_file, "                try:")
            write(gen_file, "                    obj_fq_name = self.uuid_to_fq_name(obj_uuid)")
            write(gen_file, "                    ret_list.append((obj_fq_name, obj_uuid))")
            write(gen_file, "                except cfgm_common.exceptions.NoIdError:")
            write(gen_file, "                    pass")
            write(gen_file, "            return ret_list")
            write(gen_file, "        # end get_fq_name_uuid_list")
            write(gen_file, "")
            ...
            write(gen_file, "")
            write(gen_file, "            def filter_rows_parent_anchor(sort=False):")
            write(gen_file, "                # flatten to [('children:<type>:<uuid>', (<val>,<ts>), *]")
            write(gen_file, "                all_cols = [cols for obj_key in obj_rows.keys() for cols in obj_rows[obj_key].items()]")
            write(gen_file, "                all_child_infos = {}")
            write(gen_file, "                for col_name, col_val_ts in all_cols:")
            write(gen_file, "                    # give chance for zk heartbeat/ping")
            write(gen_file, "                    gevent.sleep(0)")
            write(gen_file, "                    child_uuid = col_name.split(':')[2]")
            write(gen_file, "                    if obj_uuids and child_uuid not in obj_uuids:")
            write(gen_file, "                        continue")
            write(gen_file, "                    all_child_infos[child_uuid] = {'uuid': child_uuid, 'tstamp': col_val_ts[1]}")
            write(gen_file, "")
            write(gen_file, "                filter_cols = ['prop:%s' %(fname) for fname, _ in filter_fields]")
            write(gen_file, "                if filter_cols:")
            write(gen_file, "                    filt_child_infos = filter_rows(all_child_infos, filter_cols, filter_fields)")
            write(gen_file, "                else: # no filter specified")
            write(gen_file, "                    filt_child_infos = all_child_infos")
            write(gen_file, "")
            write(gen_file, "                if not sort:")
            write(gen_file, "                    ret_child_infos = filt_child_infos.values()")
            write(gen_file, "                else:")
            write(gen_file, "                    ret_child_infos = sorted(filt_child_infos.values(), key=itemgetter('tstamp'))")
            write(gen_file, "")
            write(gen_file, "                return get_fq_name_uuid_list(r['uuid'] for r in ret_child_infos)")
            write(gen_file, "            # end filter_rows_parent_anchor")
            write(gen_file, "")
            write(gen_file, "            if count:")
            write(gen_file, "                return (True, len(filter_rows_parent_anchor()))")
            write(gen_file, "")
            write(gen_file, "            children_fq_names_uuids = filter_rows_parent_anchor(sort=True)")
            write(gen_file, "")
            ...
            write(gen_file, "                    return (True, children_fq_names_uuids)")
            write(gen_file, "")
            write(gen_file, "            def filter_rows_backref_anchor():")
            write(gen_file, "                # flatten to [('backref:<type>:<uuid>', (<val>,<ts>), *]")
            write(gen_file, "                all_cols = [cols for obj_key in obj_rows.keys() for cols in obj_rows[obj_key].items()]")
            write(gen_file, "                all_backref_infos = {}")
            write(gen_file, "                for col_name, col_val_ts in all_cols:")
            write(gen_file, "                    # give chance for zk heartbeat/ping")
            write(gen_file, "                    gevent.sleep(0)")
            write(gen_file, "                    backref_uuid = col_name.split(':')[2]")
            write(gen_file, "                    if obj_uuids and backref_uuid not in obj_uuids:")
            write(gen_file, "                        continue")
            write(gen_file, "                    all_backref_infos[backref_uuid] = {'uuid': backref_uuid, 'tstamp': col_val_ts[1]}")
            write(gen_file, "")
            write(gen_file, "                filter_cols = ['prop:%s' %(fname) for fname, _ in filter_fields]")
            write(gen_file, "                if filter_cols:")
            write(gen_file, "                    filt_backref_infos = filter_rows(all_backref_infos, filter_cols, filter_fields)")
            write(gen_file, "                else: # no filter specified")
            write(gen_file, "                    filt_backref_infos = all_backref_infos")
            write(gen_file, "")
            write(gen_file, "                return get_fq_name_uuid_list(r['uuid'] for r in filt_backref_infos.values())")
            write(gen_file, "            # end filter_rows_backref_anchor")
            write(gen_file, "")
            write(gen_file, "            if count:")
            write(gen_file, "                return (True, len(filter_rows_backref_anchor()))")
            write(gen_file, "")
            write(gen_file, "            children_fq_names_uuids = filter_rows_backref_anchor()")
            write(gen_file, "")
            write(gen_file, "        if not parent_uuids and not back_ref_uuids:")
            write(gen_file, "            obj_uuid_cf = self._obj_uuid_cf")
            write(gen_file, "            if obj_uuids:")
            write(gen_file, "                # exact objects specified")
            write(gen_file, "                def filter_rows_object_list():")
            write(gen_file, "                    all_obj_infos = {}")
            write(gen_file, "                    for obj_uuid in obj_uuids:")
            write(gen_file, "                        all_obj_infos[obj_uuid] = None")
            write(gen_file, "")
            write(gen_file, "                    filter_cols = ['prop:%s' %(fname) for fname, _ in filter_fields]")
            write(gen_file, "                    if filter_cols:")
            write(gen_file, "                        filt_obj_infos = filter_rows(all_obj_infos, filter_cols, filter_fields)")
            write(gen_file, "                    else: # no filters specified")
            write(gen_file, "                        filt_obj_infos = all_obj_infos")
            write(gen_file, "")
            write(gen_file, "                    return get_fq_name_uuid_list(filt_obj_infos.keys())")
            write(gen_file, "                # end filter_rows_object_list")
            write(gen_file, "")
            write(gen_file, "                if count:")
            write(gen_file, "                    return (True, len(filter_rows_object_list()))")
            write(gen_file, "                children_fq_names_uuids = filter_rows_object_list()")
            write(gen_file, "")
            write(gen_file, "            else: # grab all resources of this type")
            ...
            write(gen_file, "                        return (True, children_fq_names_uuids)")
            write(gen_file, "")
            write(gen_file, "                def filter_rows_no_anchor():")
            write(gen_file, "                    all_obj_infos = {}")
            ...
            write(gen_file, "                        all_obj_infos[obj_uuid] = (col_name_arr[:-1], obj_uuid)")
            write(gen_file, "")
            write(gen_file, "                    filter_cols = ['prop:%s' %(fname) for fname, _ in filter_fields]")
            write(gen_file, "                    if filter_cols:")
            write(gen_file, "                        filt_obj_infos = filter_rows(all_obj_infos, filter_cols, filter_fields)")
            ...
            write(gen_file, "                        filt_obj_infos = all_obj_infos")
            write(gen_file, "")
            write(gen_file, "                    return filt_obj_infos.values()")
            write(gen_file, "                # end filter_rows_no_anchor")
            write(gen_file, "")
            write(gen_file, "                if count:")
            write(gen_file, "                    return (True, len(filter_rows_no_anchor()))")
            write(gen_file, "")
            write(gen_file, "                children_fq_names_uuids = filter_rows_no_anchor()")
            write(gen_file, "")
            write(gen_file, "        return (True, children_fq_names_uuids)")
