...
class IFMapApiGenerator(object):
    ...
    def _generate_common_classes(self, gen_filepath_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "        if not fields_to_serialize: # all fields are serialized")
    ...
    def _generate_client_classes(self, gen_filepath_pfx, gen_filename_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "    #end set_uuid")
            write(gen_file, "")
            write(gen_file, "    def set_name(self, new_name):")
            write(gen_file, "        self.fq_name[-1] = new_name")
            write(gen_file, "        self.name = new_name")
            write(gen_file, "        if 'fq_name' not in self._pending_field_updates:")
            write(gen_file, "            self._pending_field_updates.add('fq_name')")
            write(gen_file, "    #end set_name")
            write(gen_file, "")
            ...
            for back_link_info in ident.getBackLinksInfo():
                ...
                write(gen_file, "        if not back_refs: # read it for first time")
...
'''
    ...
    def _generate_server_impl(self, gen_fname, gen_type_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "")
            write(gen_file, "        try:")
            write(gen_file, "            self._extension_mgrs['resourceApi'].map_method('pre_%s_delete', read_result)" %(method_name))
    ...
    def _generate_extension_impl(self, gen_fname, gen_type_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "")
            write(gen_file, "    def post_%s_create(self, resource_id, resource_dict):" %(method_name))
            write(gen_file, '        """')
            ...
            write(gen_file, "")
            write(gen_file, "    def pre_%s_update(self, resource_id):" %(method_name))
            write(gen_file, '        """')
            ...
            write(gen_file, "")
            write(gen_file, "    def post_%s_update(self, resource_id):" %(method_name))
            write(gen_file, '        """')
    ...
    def _generate_cassandra_db_impl(self, gen_fname, gen_type_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "        for col_name in obj_cols.keys():")
            write(gen_file, "            # TODO use compiled RE")
            write(gen_file, "            if re.match('fq_name', col_name):")
            write(gen_file, "                cur_fq_name_json = obj_cols['fq_name']")
            write(gen_file, "                if 'fq_name' in new_obj_dict:")
            write(gen_file, "                    new_fq_name_json = json.dumps(new_obj_dict['fq_name'])")
            write(gen_file, "                    if cur_fq_name_json != new_fq_name_json:")
            write(gen_file, "                        bch.insert(obj_uuid, {'fq_name': new_fq_name_json})")
            write(gen_file, "")
            write(gen_file, "            if re.match('prop:', col_name):")
            write(gen_file, "                (_, prop_name) = col_name.split(':')")
            ...
            write(gen_file, "        bch.send()")
            write(gen_file, "")
            write(gen_file, "        # Update fqname table if there was a name change")
            write(gen_file, "        if 'fq_name' in new_obj_dict:")
            write(gen_file, "            new_fq_name_json = json.dumps(new_obj_dict['fq_name'])")
            write(gen_file, "            if cur_fq_name_json != new_fq_name_json:")
            write(gen_file, "                # delete old column")
            write(gen_file, "                cur_fq_name_str = ':'.join(json.loads(cur_fq_name_json))")
            write(gen_file, "                fq_name_col = cur_fq_name_str + ':' + obj_uuid")
            write(gen_file, "                self._obj_fq_name_cf.remove('%s', columns = [fq_name_col])" %(method_name))
            write(gen_file, "")
            write(gen_file, "                # add new column")
            write(gen_file, "                new_fq_name_str = ':'.join(new_obj_dict['fq_name'])")
            write(gen_file, "                fq_name_cols = {new_fq_name_str + ':' + obj_uuid: json.dumps(None)}")
            write(gen_file, "                self._obj_fq_name_cf.insert('%s', fq_name_cols)" %(method_name))
            write(gen_file, "")
            write(gen_file, "        return (True, '')")
