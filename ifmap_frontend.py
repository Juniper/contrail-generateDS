...
'''
    ...
    def _generate_client_impl(self, gen_fname, gen_type_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "             ref_orig = set([(x['uuid'], x.get('attr')) for x in getattr(obj, '_original_' + ref_name, [])])")
            write(gen_file, "             ref_new = set([(x['uuid'], x.get('attr')) for x in getattr(obj, ref_name, [])])")
            write(gen_file, "             for ref in ref_orig - ref_new:")
            write(gen_file, "                 self.ref_update('%s', obj.uuid, ref_name, ref[0], 'DELETE')" %(ident_name))
            write(gen_file, "             for ref in ref_new - ref_orig:")
            write(gen_file, "                 self.ref_update('%s', obj.uuid, ref_name, ref[0], 'ADD', ref[1])" %(ident_name))
